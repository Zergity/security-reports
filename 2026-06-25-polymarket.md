...
> Scope note: This is an internal reference report for security analysis. The fund flow (§4), the drain transactions (§4.7), and the attack mechanism (§3.3) were independently verified on-chain (Etherscan V2 API and `debug_traceTransaction`, 2026-07-01); those findings are first-hand. Off-chain context (vendor identity, script IoCs) remains as-reported and is flagged where unconfirmed.
...
4. **Exploitation** — In the victim's browser, the script prompted **unintended signatures**. On-chain tracing (§3.3, confirmed) shows these were **Polymarket proxy-wallet execution authorizations**: each victim signed a message that let their Polymarket proxy wallet execute a transaction, which the attacker relayed to make the proxy `transfer` its pUSD to an attacker-controlled collector.
...
### 3.3 Signing primitive — CONFIRMED via on-chain trace
We located the Polygon drain transactions and ran `debug_traceTransaction` (callTracer) on them (see §4.7). The mechanism is **not** a standard ERC-20 `approve`/`permit` drainer — it is **abuse of Polymarket's proxy-wallet (account-abstraction) signed-execution flow**. Every drain has the identical call tree:
...
### 3.5 Decoded signed payload (recovered on-chain)
Worked example — the largest drain, tx `0x2ca925546092ed6e121e3ed8aec26b98b3c176a4bb3c1e2bf76caa1a21a2b0cc`. Recovered from **bytecode + `debug_traceTransaction`** only (no off-chain data).

**Verifying function** (proxy impl `DepositWallet` `0x58ca52eb…`, verified on Polygonscan, selector `0xe8c8bf64`):
```solidity
execute(
...
**Signed struct values** (decoded from the `execute` calldata via callTracer):
```
...
```
Canonical `EIP712Domain` typehash `0x8b73c3c6…400f` appears as a `PUSH32` constant in the impl bytecode, confirming the scheme.

**Method notes / reproducibility:**
- Struct *values*, digest, signature, and signer are recoverable from any drain via calldata + the `ecrecover` precompile call — `callTracer` suffices (ecrecover is a precompile CALL, not an internal keccak, so no `structLog` needed).
- Field *names/types* came from the verified `DepositWallet` ABI. If it were unverified: extract the struct `TYPEHASH` `PUSH32` from bytecode and match against known type strings (keccak is not invertible); values still decode from calldata by layout.
...
The signature was requested via **`eth_signTypedData_v4`** (nested `Call[]` array → requires v4). The reconstructed typed data below was **verified by recomputing the EIP-712 digest** — it hashes to exactly the on-chain `0x582080cc…`, so this is precisely what was signed.
...
### 3.7 Supply-chain vector — what the client-side captures show
Attempted to recover the injected script from public **urlscan.io** captures of `polymarket.com` (queried 2026-07-01). Facts found:
...
- The **08:05 during-window DOM contained none** of the attacker indicators (collector addresses, `signTypedData`, `DepositWallet`, in-page `eval`/`atob`). The headless crawler (no wallet) was **not served the drainer payload**.
...
- **Result:** the malicious drainer script was **not recoverable from public crawler data** — the injection was evidently gated to real wallet sessions (which headless crawlers don't trigger). The compromised vendor and script IoCs (URL / hash) remain **unidentified** (see §3.4). Recovering them would require a real wallet-session capture (a victim's browser, or a client-side monitoring vendor) or Polymarket's own post-mortem.

---

## 4. On-chain fund flow (independently verified)

> **Method:** Ethereum + Polygon queried via the **Etherscan V2 API** (`chainid` 1 / 137) for token transfers and log topics; balances/nonces/traces via RPC `eth.drpc.org` and `polygon.drpc.org` (`debug_traceTransaction`). All checked **2026-07-01**. Attacker consolidation address first attributed by sleuths **Specter** / **AMLBot**; the Polygon collectors, victim proxies, drain txs, and call-traces below are our own reconstruction.
...
- **Tracer-confusion / address-poisoning:** cluster addresses shuffle **fake ERC-20 tokens named "ETH"** using Unicode homoglyphs (`E឵Τ឵H`, `ĖTḨ`) in amounts mirroring the real ETH flow — designed to pollute explorer views and automated tracers.
...
**Attacker contracts / IoCs (Polygon):**
- Drainer contract: `0x00000000000fb5c9adea0298d729a0cb3823cc07` (leading-zeros vanity, 124-byte minimal contract) → impl `0xb6f9c7e6…`
- Executor EOAs (rotating): `0xde9e08b97b6b0c5b2aab2fac11a4f2d0f0288c3b`, `0x4c917200bda6cb9b2608cc07029f846a66d06395`, …
- Victim wallets are Polymarket **proxy** wallets (delegatecall to Polymarket proxy impl `0x58ca52eb…`).
...
- **On-chain (first-hand):** Etherscan V2 API (`chainid` 1/137) for token transfers + log topics; `eth.drpc.org` / `polygon.drpc.org` for balances and `debug_traceTransaction` (callTracer) — queried 2026-07-01. Drain-tx set and traces reconstructed independently.
