...
| 2026-07-11 | Bonzo Lend / Supra oracle (Hedera) | ~$9.05M (6,634,528.20 USDC + 34,518,389.36 WHBAR); + ~$1M by a self-styled white-hat | Oracle verifier flaw, not a Bonzo bug: Supra's pull-oracle BLS-over-BN254 verifier returned a zero (identity-element) public key for an unpopulated committee (ID 2) and accepted a zeroed signature `[0,0]`, so the pairing check `e(σ,g₂)==e(H(m),pk)` collapsed to `1==1` for any message. Attacker wrote a SAUCE price of `10³⁰` (~12 OoM high) into pair 425, then borrowed the pool against a 250-SAUCE deposit. Verified first-hand on Hedera: decoded the malicious payload (committee 2, zero sig, price `10³⁰`), the call graph through the verifier to the `0x08` pairing precompile (returned success), the 250-SAUCE deposit, and both borrows to the exact digit | [report](2026-07-11-bonzo-supra.md) |
